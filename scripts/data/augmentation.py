...
class RandomColorJit(object):
  """
    Random color jitting for image
  """
  def __call__(self, image, target):
    transformer = transforms.ColorJitter(0.25, 0.25, 0.25, 0.1)
    image = np.asarray(transformer(Image.fromarray(image)))
...
if __name__ == '__main__':
  ...
  img_aug = np.asarray(transforms.ColorJitter(0.25, 0.25, 0.25, 0.1)(Image.fromarray(img)))
